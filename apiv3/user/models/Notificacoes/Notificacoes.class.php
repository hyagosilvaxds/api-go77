...
class Notificacoes extends Conexao
{
  ...
  public function save($type, $id, $param){
      ...
      switch ($type) {
        ...
        case 'reserva-cancelada-anfitriao':
          $gcm->notificacao_android_user($titulo, $descricao,$id);
          $gcm->notificacao_ios_user($titulo, $descricao,$id);
        break;
  ...
  public function enviaMensagemUser($titulo,$descricao,$id){
      ...
      $gcm =  new Gcm();
       ...
       $gcm->notificacao_android_user("Novo orçamento","um fornecedor te enviou um novo orçamento",$listAllOrcamentos[0]['id_cliente']);
       $gcm->notificacao_ios_user("Novo orçamento","um fornecedor te enviou um novo orçamento",$listAllOrcamentos[0]['id_cliente']);
  }
  ...
  //   }
    ...
    public function saveNotificacoes($titulo,$descricao)
    {
        ...
        $sql = $this->mysqli->prepare("INSERT INTO app_notificacoes (app_users_id,titulo, descricao, data) VALUES ('0','$titulo', '$descricao', '$this->data')");
        $sql->execute();
  ...
  //       $sql = $this->mysqli->prepare("SELECT id, titulo, descricao, data FROM app_notificacoes WHERE app_users_id='0'");
