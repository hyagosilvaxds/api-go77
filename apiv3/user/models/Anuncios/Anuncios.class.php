...
class Anuncios extends Conexao
{
    ...
    public function adicionar($id_user, $id_categoria, $id_subcategoria, $nome, $descricao, $checkin, $checkout, $data_in, $data_out){

        // Trata valores vazios de data para NULL
        $data_in_sql = empty($data_in) ? "NULL" : "'$data_in'";
        $data_out_sql = empty($data_out) ? "NULL" : "'$data_out'";

        $sql = $this->mysqli->prepare(
            "INSERT INTO `app_anuncios`(`app_users_id`, `app_categorias_id`, `app_subcategorias_id`,`nome`, `descricao`, `checkin`, `checkout`, `data_in`, `data_out`, `data_cadastro`, `status`, `status_aprovado`, `finalizado`)
             VALUES ('$id_user', '$id_categoria', '$id_subcategoria', '$nome', '$descricao', '$checkin', '$checkout', $data_in_sql, $data_out_sql, '$this->data_atual', 1, 2, 2)"
        );
        $sql->execute();
    ...
    public function adicionarTypeIng($id, $tipo, $nome, $valor, $qtd){
        ...
        $Param = [
            "status" => "01",
            "id" => $id_cadastro,
            "msg" => "Tipo de ingresso adicionado com sucesso."
        ];
    ...
    public function excluirTypeIng($id)
    {


      $sql = $this->mysqli->prepare("DELETE FROM `app_anuncios_ing_types` WHERE id='$id'");
      $sql->execute();

      $Param = [
          "status" => "01",
          "msg" => "Tipo de ingresso excluído com sucesso."
      ];
    ...
    public function update($id, $id_categoria, $id_subcategoria, $nome, $descricao, $checkin, $checkout, $data_in, $data_out, $status){
      ...
      checkin='$checkin', checkout='$checkout', data_in='$data_in', data_out='$data_out', status='$status'
      WHERE id='$id'"
      );
    ...
    public function updateTypeIng($id, $tipo, $nome, $valor, $qtd){
      ...
      $Param = [
          "status" => "01",
          "msg" => "Tipo de ingresso atualizado com sucesso."
      ];
    ...
    public function updateCapaAll($id_anuncio){

      $sql = $this->mysqli->prepare("UPDATE `app_anuncios_fotos` SET capa='2' WHERE app_anuncios_id='$id_anuncio'");
      $sql->execute();
    ...
    public function adicionarImagensType($id, $url)
    {


        $sql_cadastro = $this->mysqli->prepare("
        INSERT INTO `app_anuncios_types_fotos` (`app_anuncios_types_id`, `capa`, `url`)
        VALUES ('$id', '2', '$url')");
      ...
      if ($rows == 0) {
          ...
          while ($row = $sql->fetch()) {
              ...
              if ($distancia <= $dados_config['raio_km']) {
                ...
                $Param['checkin'] = data($checkin);
                $Param['checkout'] = data($checkout);
                $Param['status'] = $status;
    ...
    public function listaFilter(
      ...
      $filter = " WHERE a.app_users_id !='$id_user' AND a.status=1 AND a.status_aprovado=1 AND a.finalizado=1";
      ...
      if(!empty($id_subcategoria)){
          $filter .= " AND a.app_subcategorias_id='$id_subcategoria'";
      }
      if(!empty($endereco)){
          $filter .= " AND c.end like '%$endereco%'";
      }
      if(!empty($adultos)){
          $filter .= " AND g.adultos >= '$adultos'";
      }
      if(!empty($criancas)){
          $filter .= " AND g.criancas >= '$criancas'";
      }
      if(!empty($quartos)){
          $filter .= " AND g.quartos >= '$quartos'";
      }
      if(!empty($banheiros)){
          $filter .= " AND g.banheiros >= '$banheiros'";
      }
      if(!empty($pets)){
          $filter .= " AND g.pets >= '$pets'";
      }
      if(!empty($caracteristicas)){
          $carac = implode(',', $caracteristicas);
          $filter .= " AND h.app_caracteristicas_id IN ($carac) ";
      }
    ...
    public function listOrcSUM($id){
      ...
      public function countAnunciosUser($id_user){

      $sql = $this->mysqli->prepare("SELECT count(*) FROM `app_anuncios` WHERE app_users_id='$id_user'");
    ...
    public function cobrarCartaoComToken($valor, $customer, $token, $nome, $cpf, $cep, $numero, $email, $celular)
{
    ...
    /**
     * Busca todas as reservas de um anúncio
     */
    public function getReservasAnuncio($id_anuncio)
    {
        $sql = $this->mysqli->prepare(
            "
            SELECT r.id, r.app_users_id, r.data_de, r.data_ate, r.adultos, r.criancas,
                   r.valor_final, r.taxa_limpeza, r.obs, r.status, r.data_cadastro,
                   u.nome as nome_hospede
            FROM `app_reservas` as r
            LEFT JOIN `app_users` as u ON r.app_users_id = u.id
            WHERE r.app_anuncios_id = ? AND r.status = 1
            ORDER BY r.data_de ASC
            "
        );
        ...
        $sql->execute();
        $sql->bind_result(
            $id, $app_users_id, $data_de, $data_ate, $adultos, $criancas,
            $valor_final, $taxa_limpeza, $obs, $status, $data_cadastro, $nome_hospede
        );
        $sql->store_result();
        ...
        if ($sql->num_rows == 0) {
            ...
            while ($sql->fetch()) {
                $reservas[] = [
                    ...
                    'nome_hospede' => $nome_hospede ? decryptitem($nome_hospede) : ''
                ];
            }
        }
    ...
    /**
     * Busca reservas de um tipo/quarto específico do anúncio
     */
    public function getReservasAnuncioPorTipo($id_anuncio, $id_type)
    {
        $sql = $this->mysqli->prepare(
            "SELECT r.id, r.app_users_id, r.data_de, r.data_ate, r.adultos, r.criancas,
                   r.valor_final, r.taxa_limpeza, r.obs, r.status, r.data_cadastro,
                   u.nome as nome_hospede, r.app_anuncios_types_id, r.app_anuncios_types_unidades_id
            FROM `app_reservas` as r
            LEFT JOIN `app_users` as u ON r.app_users_id = u.id
            WHERE r.app_anuncios_id = ? AND r.app_anuncios_types_id = ? AND r.status = 1
            ORDER BY r.data_de ASC"
        );
        
        $sql->bind_param('ii', $id_anuncio, $id_type);
        $sql->execute();
        $result = $sql->get_result();
        
        $reservas = [];
        while ($row = $result->fetch_assoc()) {
            $row['nome_hospede'] = $row['nome_hospede'] ? decryptitem($row['nome_hospede']) : '';
            $reservas[] = $row;
        }
    ...
    /**
     * Busca reservas de uma unidade específica do anúncio
     * NOTA: A tabela app_reservas ainda não tem suporte a unidades individuais.
     * Por isso, retorna as reservas do TIPO e complementa com bloqueios iCal da UNIDADE.
     * Quando a tabela for atualizada, ajustar este método.
     */
    public function getReservasAnuncioPorUnidade($id_anuncio, $id_type, $id_unidade)
    {
        // Reservas do TIPO (ainda não há suporte a unidade em app_reservas)
        // Quando a tabela for atualizada, adicionar filtro por unidade aqui
        $sql = $this->mysqli->prepare(
            "SELECT r.id, r.app_users_id, r.data_de, r.data_ate, r.adultos, r.criancas,
                   r.valor_final, r.taxa_limpeza, r.obs, r.status, r.data_cadastro,
                   u.nome as nome_hospede, r.id_anuncio_type
            FROM `app_reservas` as r
            LEFT JOIN `app_users` as u ON r.app_users_id = u.id
            WHERE r.app_anuncios_id = ? AND r.id_anuncio_type = ? AND r.status = 1
            ORDER BY r.data_de ASC"
        );
        ...
        // Também inclui bloqueios importados via iCal externo para esta UNIDADE específica
        $bloqueios = $this->getBloqueiosIcalPorUnidade($id_anuncio, $id_type, $id_unidade);
        foreach ($bloqueios as $bloqueio) {
            $reservas[] = [
                'id' => 'ical_' . $bloqueio['id'],
                'app_users_id' => 0,
                'data_de' => $bloqueio['data_inicio'],
                'data_ate' => $bloqueio['data_fim'],
                'adultos' => 0,
                'criancas' => 0,
                'valor_final' => 0,
                'taxa_limpeza' => 0,
                'obs' => $bloqueio['resumo'],
                'status' => 1,
                'data_cadastro' => $bloqueio['data_importacao'],
                'nome_hospede' => $bloqueio['resumo'] ?? 'Bloqueio iCal',
                'id_anuncio_type' => $id_type,
                'id_unidade' => $id_unidade,
                'origem' => 'ical_externo',
                'plataforma' => $bloqueio['plataforma'] ?? ''
            ];
        }
        
        // Ordena por data
        usort($reservas, function($a, $b) {
            return strtotime($a['data_de']) - strtotime($b['data_de']);
        });
    ...
    /**
     * Busca bloqueios iCal de uma unidade específica
     */
    public function getBloqueiosIcalPorUnidade($id_anuncio, $id_type, $id_unidade)
    {
        $stmt = $this->mysqli->prepare(
            "SELECT b.*, l.nome as plataforma
             FROM `app_ical_bloqueios` b
             INNER JOIN `app_ical_links` l ON b.app_ical_links_id = l.id
             WHERE b.app_anuncios_id = ? AND b.app_anuncios_types_id = ? 
                   AND b.app_anuncios_types_unidades_id = ? AND b.data_fim >= CURDATE()
             ORDER BY b.data_inicio ASC"
        );
        $stmt->bind_param("iii", $id_anuncio, $id_type, $id_unidade);
        $stmt->execute();
    ...
    /**
     * Gera o link iCal permanente para um anúncio
     * O link é fixo e sempre retorna dados atualizados quando acessado
     */
    public function gerarLinkIcal($id_anuncio)
    {
        $salt = 'go77ical_permanent_2025';
        $token = substr(md5($id_anuncio . '_' . $salt), 0, 16);
        return HOME_URI_ROOT . '/apiv3/user/anuncios/ical/' . $id_anuncio . '_' . $token . '/';
    }

    /**
     * Gera o link iCal permanente para uma UNIDADE específica
     */
    public function gerarLinkIcalUnidade($id_anuncio, $id_type, $id_unidade)
    {
        $salt = 'go77ical_unidade_2025';
        $token = substr(md5($id_anuncio . '_' . $id_type . '_' . $id_unidade . '_' . $salt), 0, 16);
        return HOME_URI_ROOT . '/apiv3/user/anuncios/icalUnidade/' . $id_anuncio . '_' . $id_type . '_' . $id_unidade . '_' . $token . '/';
    }

    /**
     * Busca todos os tipos de quarto de um anúncio
     */
    public function getTypesAnuncio($id_anuncio)
    {
        $stmt = $this->mysqli->prepare(
            "SELECT t.id, t.nome, t.descricao,
                    (SELECT v.qtd FROM `app_anuncios_valor` v WHERE v.app_anuncios_types_id = t.id LIMIT 1) as qtd
    ...
    /**
     * Gera links iCal para todas as unidades de um anúncio
     */
    public function gerarLinksIcalUnidades($id_anuncio)
    {
        $links = [];
        
        // Busca todos os tipos do anúncio
        $types = $this->getTypesAnuncio($id_anuncio);
        
        foreach ($types as $type) {
            // Garante que as unidades existam
            $this->garantirUnidadesTipo($type['id']);
            
            // Busca unidades do tipo
            $unidades = $this->listarUnidadesTipo($type['id']);
            ...
            foreach ($unidades as $unidade) {
                $links[] = [
                    ...
                    'url' => $this->gerarLinkIcalUnidade($id_anuncio, $type['id'], $unidade['id'])
                ];
            }
        }
    ...
    // ==================== ICAL EXTERNO (IMPORTAÇÃO POR QUARTO/TIPO) ====================

    /**
     * Adiciona um link iCal externo (Airbnb, Booking, etc) para um quarto/tipo específico
     */
    public function adicionarIcalExterno($id_anuncio, $nome, $url, $id_type = null)
    {
        $stmt = $this->mysqli->prepare(
            "INSERT INTO `app_ical_links` (`app_anuncios_id`, `app_anuncios_types_id`, `nome`, `url`, `data_cadastro`) 
             VALUES (?, ?, ?, ?, NOW())"
        );
        $stmt->bind_param("iiss", $id_anuncio, $id_type, $nome, $url);
        $stmt->execute();
        $id = $stmt->insert_id;
    ...
    public function removerIcalExterno($id_link, $id_anuncio)
    {
        ...
        return [
            "status" => "02",
            "msg" => "Link não encontrado ou não pertence ao anúncio."
        ];
    }

    /**
     * Lista links iCal externos de um anúncio (todos os tipos/quartos)
     */
    public function listarIcalExterno($id_anuncio, $id_type = null)
    {
        if ($id_type) {
            // Busca links de um tipo/quarto específico
            $stmt = $this->mysqli->prepare(
                "SELECT l.id, l.app_anuncios_id, l.app_anuncios_types_id, l.app_anuncios_types_unidades_id,
                        l.nome, l.url, l.ultima_sincronizacao, 
                        l.status, l.erros, l.ultimo_erro, l.data_cadastro, t.nome as nome_quarto
                 FROM `app_ical_links` l
                 LEFT JOIN `app_anuncios_types` t ON l.app_anuncios_types_id = t.id
                 WHERE l.app_anuncios_id = ? AND l.app_anuncios_types_id = ?
                 ORDER BY l.data_cadastro DESC"
            );
            $stmt->bind_param("ii", $id_anuncio, $id_type);
        } else {
            // Busca todos os links do anúncio
            $stmt = $this->mysqli->prepare(
                "SELECT l.id, l.app_anuncios_id, l.app_anuncios_types_id, l.app_anuncios_types_unidades_id,
                        l.nome, l.url, l.ultima_sincronizacao, 
                        l.status, l.erros, l.ultimo_erro, l.data_cadastro, t.nome as nome_quarto
                 ...
                 ORDER BY l.app_anuncios_types_id, l.data_cadastro DESC"
            );
            $stmt->bind_param("i", $id_anuncio);
        }
        $stmt->execute();
    ...
    /**
     * Lista links iCal externos de um tipo/quarto específico
     */
    public function listarIcalExternoPorTipo($id_type)
    {
        $stmt = $this->mysqli->prepare(
            "SELECT l.id, l.app_anuncios_id, l.app_anuncios_types_id, l.nome, l.url, 
                    l.ultima_sincronizacao, l.status, l.erros, l.ultimo_erro, l.data_cadastro
             FROM `app_ical_links` l
             WHERE l.app_anuncios_types_id = ?
             ORDER BY l.data_cadastro DESC"
        );
    ...
    /**
     * Atualiza status de sincronização de um link iCal
     */
    public function atualizarSincronizacaoIcal($id_link, $sucesso = true, $erro = null)
    {
        if ($sucesso) {
            $stmt = $this->mysqli->prepare(
            ...
            $stmt->bind_param("si", $erro, $id_link);
        }
        $stmt->execute();
    }

    /**
     * Desativa link iCal com muitos erros
     */
    public function desativarIcalComErros($id_link)
    {
        $stmt = $this->mysqli->prepare(
            "UPDATE `app_ical_links` SET `status` = 0 WHERE `id` = ?"
        );
        $stmt->bind_param("i", $id_link);
        $stmt->execute();
    }
    ...
    /**
     * Busca todos os links iCal ativos para sincronização
     */
    public function buscarLinksIcalAtivos()
    {
        $stmt = $this->mysqli->prepare(
            "SELECT l.*, a.nome as nome_anuncio 
             FROM `app_ical_links` l
             INNER JOIN `app_anuncios` a ON l.app_anuncios_id = a.id
             WHERE l.status = 1 AND l.erros < 5
    ...
    /**
     * Limpa bloqueios antigos de um link iCal
     */
    public function limparBloqueiosIcal($id_link)
    {
        $stmt = $this->mysqli->prepare(
            "DELETE FROM `app_ical_bloqueios` WHERE `app_ical_links_id` = ?"
        );
    ...
    /**
     * Adiciona um bloqueio importado via iCal (por quarto/tipo)
     */
    public function adicionarBloqueioIcal($id_anuncio, $id_type, $id_unidade, $id_link, $uid, $data_inicio, $data_fim, $resumo = null)
    {
        $stmt = $this->mysqli->prepare(
            "INSERT INTO `app_ical_bloqueios` 
             (`app_anuncios_id`, `app_anuncios_types_id`, `app_anuncios_types_unidades_id`, `app_ical_links_id`, `uid`, `data_inicio`, `data_fim`, `resumo`, `data_importacao`) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
             ON DUPLICATE KEY UPDATE `data_inicio` = VALUES(`data_inicio`), `data_fim` = VALUES(`data_fim`), 
             `resumo` = VALUES(`resumo`), `data_importacao` = NOW()"
        );
        $stmt->bind_param("iiiissss", $id_anuncio, $id_type, $id_unidade, $id_link, $uid, $data_inicio, $data_fim, $resumo);
        $stmt->execute();
    }

    /**
     * Busca bloqueios importados de um anúncio (todos os tipos)
     */
    public function getBloqueiosIcal($id_anuncio, $id_type = null)
    {
        if ($id_type) {
            $stmt = $this->mysqli->prepare(
                "SELECT b.*, l.nome as plataforma, t.nome as nome_quarto
                 FROM `app_ical_bloqueios` b
                 INNER JOIN `app_ical_links` l ON b.app_ical_links_id = l.id
                 LEFT JOIN `app_anuncios_types` t ON b.app_anuncios_types_id = t.id
                 WHERE b.app_anuncios_id = ? AND b.app_anuncios_types_id = ? AND b.data_fim >= CURDATE()
                 ORDER BY b.data_inicio ASC"
            );
            $stmt->bind_param("ii", $id_anuncio, $id_type);
        } else {
            $stmt = $this->mysqli->prepare(
                "SELECT b.*, l.nome as plataforma, t.nome as nome_quarto
                 ...
                 ORDER BY b.app_anuncios_types_id, b.data_inicio ASC"
            );
            $stmt->bind_param("i", $id_anuncio);
        }
        $stmt->execute();
    ...
    /**
     * Busca bloqueios de um tipo/quarto específico
     */
    public function getBloqueiosIcalPorTipo($id_type)
    {
        $stmt = $this->mysqli->prepare(
            "SELECT b.*, l.nome as plataforma
             FROM `app_ical_bloqueios` b
             INNER JOIN `app_ical_links` l ON b.app_ical_links_id = l.id
             WHERE b.app_anuncios_types_id = ? AND b.data_fim >= CURDATE()
             ORDER BY b.data_inicio ASC"
        );
        $stmt->bind_param("i", $id_type);
        $stmt->execute();
    ...
    /**
     * Verifica se uma data está bloqueada por iCal externo para um tipo/quarto
     */
    public function verificarBloqueioIcal($id_anuncio, $data, $id_type = null)
    {
        if ($id_type) {
            $stmt = $this->mysqli->prepare(
                "SELECT COUNT(*) as total 
                 FROM `app_ical_bloqueios` 
                 WHERE `app_anuncios_id` = ? AND `app_anuncios_types_id` = ? AND ? >= `data_inicio` AND ? < `data_fim`"
            );
            $stmt->bind_param("iiss", $id_anuncio, $id_type, $data, $data);
        } else {
            $stmt = $this->mysqli->prepare(
                "SELECT COUNT(*) as total 
                 FROM `app_ical_bloqueios` 
                 WHERE `app_anuncios_id` = ? AND ? >= `data_inicio` AND ? < `data_fim`"
            );
            $stmt->bind_param("iss", $id_anuncio, $data, $data);
        }
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    /**
     * Verifica se um período está disponível (considerando reservas + bloqueios iCal)
     */
    public function verificarDisponibilidadeCompleta($id_anuncio, $data_inicio, $data_fim)
    {
        // Verifica reservas
        $stmt = $this->mysqli->prepare(
            "SELECT COUNT(*) as total 
             FROM `app_reservas` 
             WHERE `app_anuncios_id` = ? AND `status` = 1 
             AND NOT (`data_out` <= ? OR `data_in` >= ?)"
        );
        $stmt->bind_param("iss", $id_anuncio, $data_inicio, $data_fim);
        $stmt->execute();
        $result = $stmt->get_result();
        $row = $result->fetch_assoc();
        ...
        // Verifica bloqueios iCal
        $stmt = $this->mysqli->prepare(
            "SELECT COUNT(*) as total 
             FROM `app_ical_bloqueios` 
             WHERE `app_anuncios_id` = ? 
    ...
    public function garantirUnidadesTipo($id_type)
    {
        ...
        // Verifica quantas unidades já existem
        $stmt = $this->mysqli->prepare(
    ...
    public function listarTypesComUnidades($id_anuncio)
    {
        ...
        $types = [];
        while ($row = $result->fetch_assoc()) {
            // Garante que as unidades existam e lista
    ...
    /**
     * Atualiza nome de uma unidade
     */
    public function atualizarNomeUnidade($id_unidade, $nome)
    {
        $stmt = $this->mysqli->prepare(
            "UPDATE `app_anuncios_types_unidades` SET `nome` = ? WHERE `id` = ?"
        );
        $stmt->bind_param("si", $nome, $id_unidade);
    ...
    /**
     * Lista links iCal externos com informações de unidade
     */
    public function listarIcalExternoComUnidades($id_anuncio, $id_type = null, $id_unidade = null)
    {
        $sql = "SELECT l.*, 
                       t.nome as nome_type,
                ...
                FROM `app_ical_links` l
                LEFT JOIN `app_anuncios_types` t ON l.app_anuncios_types_id = t.id
                LEFT JOIN `app_anuncios_types_unidades` u ON l.app_anuncios_types_unidades_id = u.id
                WHERE l.app_anuncios_id = ?";
        ...
        if ($id_unidade !== null && $id_unidade > 0) {
            $sql .= " AND l.app_anuncios_types_unidades_id = ?";
            $params[] = $id_unidade;
            $types .= "i";
        }
        
        $sql .= " ORDER BY l.app_anuncios_types_id, l.app_anuncios_types_unidades_id, l.id";
