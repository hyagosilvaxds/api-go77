...
class Cadastros extends Conexao {
    ...
    public function listAllCadastros($id_usuario,$nome,$data_de,$data_ate,$cidade,$estado)
    {
      ...
      if(!empty($estado)){
        ...
        $sql = $this->mysqli->prepare("
        SELECT DISTINCT a.id, a.id_grupo, a.nome, a.nome_fantasia, a.avatar, a.status, a.status_aprovado, a.data_cadastro,a.celular,a.email,b.estado,b.cidade,a.documento,a.razao_social,a.ie
        FROM app_users AS a
        INNER JOIN `app_users_endereco` AS b ON a.id=b.app_users_id
        $filter 
        ");
        $sql->execute();
        $sql->bind_result($id_usuario_cad, $id_grupo, $nome_user, $nome_fantasia,$avatar_user, $status, $status_aprovado, $data_cadastro, $celular, $email,$estado, $cidade,
         $documento, $razao_social, $ie);
        $sql->store_result();
        $rows = $sql->num_rows;
        ...
        if ($rows == 0) {
            ...
            while ($row = $sql->fetch()) {
                ...
                $Param['avatar_user'] = $avatar_user;
                $Param['nome'] = decryptitem($nome_user);
                $Param['nome_fantasia'] = decryptitem($nome_fantasia);
                $Param['status'] = $status;
