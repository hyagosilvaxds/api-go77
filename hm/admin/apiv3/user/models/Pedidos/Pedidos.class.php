...
class Pedidos extends Conexao {
    ...
    public function listAllPedidos($id_usuario,$nome,$data_de,$data_ate,$cidade,$estado)
    {
      $filter = "WHERE c.status IN (1,2,3,4,6) ";
      ...
      if(!empty($estado)){
        ...
        $sql = $this->mysqli->prepare("
        SELECT DISTINCT c.id, d.nome, c.descricao, c.data_cadastro, c.status ,b.estado, b.cidade, a.nome_fantasia,a.avatar
        ...
        $sql->execute();
        $sql->bind_result($id_cotacao, $categoria_nome, $descricao,$data_cadastro, $status, $estado, $cidade, $nome_user, $avatar);
        $sql->store_result();
        $rows = $sql->num_rows;
    ...
    public function gerarCSV() {
        ...
        $arquivo = fopen("uploads/planilhas/exportarPedidos.csv", "w");
    ...
    public function planoUser($id)
{
    ...
    //     $PedidosModel['total_anuncios'] = $total_anuncios;
    //     $PedidosModel['total_assinaturas'] = $total_assinaturas;
    //     $PedidosModel['total_turbinar'] = $total_turbinar;
    //     $PedidosModel['valor_assinatura'] ='R$' . $valor_assinaturas;
    //     $PedidosModel['valor_turbinar'] ='R$' . $valor_turbinar;
