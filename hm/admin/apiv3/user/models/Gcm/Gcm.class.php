...
class Gcm extends Conexao
{
  ...
  public function __construct()
  {
    ...
    $this->id_projeto = "492986075269";
  }


  public function notificacao_android_user($titulo, $descricao, $id_user = null, $id_grupo = null, $estado = null, $cidade = null, $isIOS = false)
  {
      ...
      if (!empty($id_user)) {
          ...
          if (!empty($id_grupo)) {
              $filter .= " AND u.id_grupo = '$id_grupo'";
          }
          if (!empty($estado)) {
              $estado = cryptitem($estado);
              $filter .= " AND e.estado = '$estado' AND e.favorito = '1'";
          }
          if (!empty($cidade)) {
              $cidade = cryptitem($cidade);
              $filter .= " AND e.cidade = '$cidade' AND e.favorito = '1'";
          }
          ...
          $query_push = "
              ...
              JOIN `app_users_endereco` AS e ON fcm.app_users_id = e.app_users_id
              $filter
          ";
      }
      ...
      if (sizeof($registrationIDs) > 1000) {
          ...
          foreach ($newId as $inner_id) {
              $this->sendNotification($url, $inner_id, $titulo, $descricao, $accessToken, $isIOS);
          }
      } else {
          $this->sendNotification($url, $registrationIDs, $titulo, $descricao, $accessToken, $isIOS);
      }
  }

  public function notificacao_ios_user($titulo, $descricao, $id_user = null, $isIOS = true)
  {
      // Verifica se o id_user foi passado
      if (!empty($id_user)) {
          // Query para selecionar o registration_id de um usuário específico
...
private function sendNotification($url, $registrationIDs, $titulo, $descricao, $accessToken, $isIOS)
{
  foreach ($registrationIDs as $registrationID) {
      ...
      if ($isIOS) {
          ...
          $message['data'] = array(
              'extra_info' => 'Aqui você pode adicionar dados extras, se necessário',
          );
      }
      $fields = array(
          'message' => $message
