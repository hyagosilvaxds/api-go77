<script>
  ...
  new Vue({
    ...
    data: {
      ...
      listallplanos: [],
      listallcupons: [],
      id_usuario: window.location.href.split("franqueadosD/")[1].split("#")[0],
      nome_visita: null,
      numero_estudo: null,
      saldo_atual:null,
      ...
      estado_modal2: null,
      cidade_modal2: null,
      endereco_modal2: null,
      bairro_modal2: null,
      numero_modal2: null,
      complemento_modal2: null,
  ...
  viacep2(){
console.log(teste)
if( this.cep_modal2.length > 8){
  let url = `https://viacep.com.br/ws/${this.cep_modal2}/json/`
  axios.get(url)
  .then((res)=> {
    console.log(res.data)
    this.estado_modal2=res.data.uf
    this.cidade_modal2=res.data.localidade
    this.bairro_modal2=res.data.bairro
    this.complemento_modal2=res.data.complemento
    this.endereco_modal2=res.data.logradouro          
  })
}
},
      saveEndereco: function() {
          axios.post('<?php echo API_ROOT ?>/usuarios/saveendereco/', {
            id_user: this.id_usuario,
              token: '<?php echo TK_ROOT ?>', cep:this.cep_modal2, estado:this.estado_modal2, cidade:this.cidade_modal2, bairro:this.bairro_modal2,
			    endereco:this.endereco_modal2, numero:this.numero_modal2, complemento:this.complemento_modal2
            })
            .then(res => {
  ...
  EstatisticasGerais: function() {
    axios.post('<?php echo API ?>/franqueados/EstatisticasGerais/', {
      id_usuario:this.id_usuario,
        token: '<?php echo TK ?>'
      })
      .then(res => {
