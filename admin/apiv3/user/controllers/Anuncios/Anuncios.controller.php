...
class AnunciosController
{
    ...
    /**
     * Gera o token iCal para um anúncio
     */
    private function gerarTokenIcal($id_anuncio)
    {
        return substr(md5($id_anuncio . '_go77ical_secret'), 0, 16);
    }

    /**
     * Retorna o link iCal de um anúncio específico (Admin pode acessar qualquer anúncio)
     * POST: { token, id_anuncio }
     */
    public function getLinkIcal()
    {
        ...
        $model = new Anuncios();
        $anuncio = $model->listID($id_anuncio);
        ...
        $token_ical = $this->gerarTokenIcal($id_anuncio);
        $base_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'];
        $link_ical = $base_url . "/apiv3/user/anuncios/ical/{$id_anuncio}_{$token_ical}/";
    ...
    /**
     * Retorna os links iCal de todos os anúncios (Admin)
     * POST: { token }
     */
    public function getTodosLinksIcal()
    {
        $this->secure->tokens_secure($this->input->token);

        $model = new Anuncios();
        $anuncios = $model->lista();
        ...
        foreach ($anuncios as $anuncio) {
            $id = $anuncio['id'];
            $token_ical = $this->gerarTokenIcal($id);
            $links[] = [
                "id_anuncio" => $id,
                "nome" => $anuncio['nome'] ?? '',
                "id_user" => $anuncio['app_users_id'] ?? '',
                "link_ical" => $base_url . "/apiv3/user/anuncios/ical/{$id}_{$token_ical}/"
            ];
        }
    ...
    /**
     * Exporta o arquivo iCal diretamente (Admin pode exportar qualquer anúncio)
     * POST: { token, id_anuncio }
     */
    public function exportarIcal()
    {
        ...
        header('Content-Disposition: attachment; filename="anuncio_' . $id_anuncio . '.ics"');
        echo $ical;
        exit;
    }
    ...
    {
        $nome_anuncio = $anuncio['nome'] ?? 'Anúncio';
        $id_anuncio = $anuncio['id'];

        $ical = "BEGIN:VCALENDAR\r\n";
        $ical .= "VERSION:2.0\r\n";
        $ical .= "PRODID:-//Go77//Reservas//PT\r\n";
        $ical .= "CALSCALE:GREGORIAN\r\n";
        $ical .= "METHOD:PUBLISH\r\n";
        $ical .= "X-WR-CALNAME:" . $this->escapeIcal($nome_anuncio) . "\r\n";
        $ical .= "X-WR-TIMEZONE:America/Sao_Paulo\r\n";

        if (!empty($reservas)) {
            foreach ($reservas as $reserva) {
                $uid = "reserva-{$reserva['id']}@go77.app";
                $data_in = isset($reserva['data_in']) ? date('Ymd', strtotime($reserva['data_in'])) : '';
                $data_out = isset($reserva['data_out']) ? date('Ymd', strtotime($reserva['data_out'] . ' +1 day')) : '';
                $nome_hospede = $reserva['nome_hospede'] ?? 'Reserva';
                $created = isset($reserva['data_cadastro']) ? date('Ymd\THis\Z', strtotime($reserva['data_cadastro'])) : date('Ymd\THis\Z');

                if (!empty($data_in) && !empty($data_out)) {
                    $ical .= "BEGIN:VEVENT\r\n";
                    $ical .= "UID:{$uid}\r\n";
                    $ical .= "DTSTART;VALUE=DATE:{$data_in}\r\n";
                    $ical .= "DTEND;VALUE=DATE:{$data_out}\r\n";
                    $ical .= "SUMMARY:" . $this->escapeIcal("Reserva - " . $nome_hospede) . "\r\n";
                    $ical .= "DESCRIPTION:" . $this->escapeIcal("Reserva no anúncio: " . $nome_anuncio) . "\r\n";
    ...
    /**
     * Escapa caracteres especiais para iCal
     */
    ...
    /**
     * Adiciona um link iCal externo (Admin pode adicionar em qualquer anúncio)
     * 
     * Payload:
     * - id_anuncio: ID do anúncio
     * - id_type: ID do quarto/tipo (obrigatório se anúncio tiver quartos, 0 ou null se não tiver)
    ...
    public function adicionarIcalExterno()
    {
        ...
        if ($qtdTypes > 0) {
            ...
            // Anúncio NÃO tem quartos - id_type deve ser 0
            $id_type = 0;
        }
    ...
    /**
     * Lista links iCal externos de um anúncio (Admin)
     * 
     * Payload:
     * - id_anuncio: ID do anúncio
    ...
    /**
     * Remove um link iCal externo (Admin)
     */
    ...
    /**
     * Lista bloqueios importados via iCal (Admin)
     * 
     * Payload:
     * - id_anuncio: ID do anúncio
     * - id_type: ID do quarto/tipo (opcional - se não informado, lista todos)
