...
class Cadastros extends Conexao {
    ...
    public function listAllCadastros($id_usuario,$nome,$data_de,$data_ate,$cidade,$estado)
    {

      $nome_final = cryptitem($nome);


      $filter = "WHERE a.id_grupo='4' ";

      if(!empty($nome)){$filter .= "AND a.nome LIKE '%$nome_final%'";}
      if(!empty($data_de)){$filter .= "AND a.data_cadastro>='$data_de'";}
      if(!empty($data_ate)){$filter .= "AND a.data_cadastro<='$data_ate 23:59:59' ";}

        $sql = $this->mysqli->prepare("
        SELECT DISTINCT a.id, a.id_grupo, a.tipo_pessoa, a.nome, a.avatar, a.status, a.status_aprovado, a.data_cadastro,a.celular,a.email, a.documento, a.cnpj, b.latitude,b.longitude
        FROM app_users AS a
        INNER JOIN `app_users_location` AS b ON a.id=b.app_users_id
        $filter
        ");
        $sql->execute();
        $sql->bind_result($id_usuario_cad, $id_grupo, $tipo_pessoa, $nome_user, $avatar_user, $status, $status_aprovado, $data_cadastro, $celular, $email, $documento, $cnpj, $latitude, $longitude);
        $sql->store_result();
        $rows = $sql->num_rows;
        ...
        if ($rows == 0) {
            ...
            while ($row = $sql->fetch()) {
                ...
                $Param['id_usuario'] = $id_usuario_cad;
                $Param['id_grupo'] = $id_grupo;
                $Param['tipo_pessoa'] = tipoPessoa($tipo_pessoa);
                $Param['avatar_user'] = $avatar_user;
                $Param['nome'] = decryptitem($nome_user);
                $Param['status'] = $status;
                $Param['documento'] = decryptitem($documento);
                $Param['cnpj'] = decryptitem($cnpj);
    ...
    public function gerarCSV() {
        ...
        $arquivo = fopen("uploads/planilhas/exportarCadastros.csv", "w");
    ...
    public function Perfil($id)
    {
        ...
        a.data_cadastro,a.u_login,a.token_senha,a.status,a.status_aprovado, a.documento,a.cnpj,a.razao_social,a.nome_fantasia,
        a.ie, a.perc_imoveis, a.perc_eventos, b.tipo_chave,b.chave,
        c.saldo
        FROM `app_users` AS a
        ...
        if ($rows == 0) {
            ...
            $success['id'] = $this->id;
            $success['tipo_pessoa'] = tipoPessoa($this->tipo_pessoa);
    ...
    public function planoUser($id)
{
    ...
    public function updateCadastro($id_usuario,$nome,$email,$celular,$documento,$data_nascimento,$perc_imoveis,$perc_eventos,$status_aprovado,$status)
    {
        $sql_cadastro = $this->mysqli->prepare("UPDATE app_users SET nome='$nome', email='$email', celular='$celular', documento='$documento',
        data_nascimento='$data_nascimento', perc_imoveis='$perc_imoveis', perc_eventos='$perc_eventos',status_aprovado='$status_aprovado',status='$status' WHERE id='$id_usuario'");
    ...
    public function updateCadastro_bkp($id_usuario, $nome, $link, $saq, $status_aprovado)
    {
        ...
        $sql = $this->mysqli->prepare("
            SELECT
                (SELECT COUNT(a.id)
                 ...
                 WHERE b.id = '$id' AND a.status_aprovado='1') AS anuncios_ativos,

                 (SELECT COUNT(a.id)
                  FROM app_anuncios AS a
                  INNER JOIN app_users AS b ON a.app_users_id = b.id
                  WHERE b.id = '$id' AND a.status_aprovado='2') AS anuncios_pendentes,

                (SELECT SUM(a.valor_final)
                 FROM app_pagamentos AS a
                 INNER JOIN app_users AS b ON a.app_users_id = b.id
                 WHERE b.id = '$id' AND a.status IN('RECEIVED', 'CONFIRMED')) AS pagamentos_total,

                 (SELECT COUNT(a.id)
                  FROM app_avaliacoes AS a
                  INNER JOIN app_users AS b ON a.app_users_id = b.id
                  WHERE b.id = '$id') AS avaliacoes,

                  (SELECT COUNT(a.id)
                   FROM app_favoritos AS a
                   INNER JOIN app_users AS b ON a.app_users_id = b.id
                   WHERE b.id = '$id') AS favoritos,

                   (SELECT COUNT(a.id)
                    FROM app_notificacoes AS a
                    INNER JOIN app_users AS b ON a.app_users_id = b.id
                    WHERE b.id = '$id') AS push,

                    (SELECT COUNT(a.id)
                     FROM app_reservas AS a
                     INNER JOIN app_users AS b ON a.app_users_id = b.id
                     WHERE b.id = '$id') AS reservas,

                     (SELECT type
                      FROM app_fcm AS a
                      INNER JOIN app_users AS b ON a.app_users_id = b.id
                      WHERE b.id = '$id') AS type_fcm

        ");


        $sql->execute();
        $sql->bind_result($anuncios_ativos, $anuncios_pendentes, $pagamentos_total, $avaliacoes, $favoritos, $push, $reservas, $type_fcm);
        $sql->fetch();
        $sql->close();

        if($type_fcm == 1){$tipo_fcm = "Android";}else{$tipo_fcm = "Iphone";}

        // Prepara o array de retorno
        $FornecedoresModel = [
            'anuncios_ativos' => $anuncios_ativos,
            'anuncios_pendentes' => $anuncios_pendentes,
            'pagamentos_total' => $pagamentos_total > 0? $pagamentos_total : 0,
