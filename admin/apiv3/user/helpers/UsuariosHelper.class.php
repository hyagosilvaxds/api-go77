         ...
         if (crypt($password, $this->password) === $this->password) {
            if ($this->status == 2) {
                ...
                return $error;
                    ...
                    // DESABILITADO TEMPORARIAMENTE - ENVIO DE EMAIL 2FA
                    // $mail = new EnviarEmail();
                    // $mail->doisFatores(decryptitem($this->nome), decryptitem($email), $codigo);
                    
                    $success['status'] = '01';
                    $success['msg'] = 'MODO DEV: Use o código ' . $codigo . ' para entrar.';
                    $success['msg2'] = $codigo;
                    $success['codigo_dev'] = $codigo; // Código visível para desenvolvimento
